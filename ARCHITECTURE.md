...
Linux-лаунчер использует Bash и приложения формата `.desktop`. Он:

- поддерживает Ubuntu и основанные на Ubuntu версии Linux Mint;
- создаёт `.env` с правами `600` и устанавливает зависимости через `npm ci`;
- ищет Antigravity в `PATH`, стандартных каталогах и среди пользовательских AppImage;
- запускает CDP только на loopback и не закрывает уже открытую Antigravity;
- записывает PID вместе со временем старта процесса и перед остановкой сверяет `/proc`, каталог, исполняемый файл и командную строку;
- оставляет Antigravity открытой при остановке AG2R.
